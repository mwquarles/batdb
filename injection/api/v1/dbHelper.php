...
class dbHelper {
    ...
    function select($table, $columns, $where){
        try{
            ...
            $stmt = $this->db->prepare("select ".$columns." from ".$table." where 1=1 ". $w . " order by id DESC limit 100");
            $stmt->execute($a);
    ...
    function select2($table, $columns, $where, $order){
        try{
            ...
            $stmt = $this->db->prepare("select ".$columns." from ".$table." where 1=1 ". $w." ".$order." limit 100");
            $stmt->execute($a);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            if(count($rows)<=0){
                $response["status"] = "warning";
